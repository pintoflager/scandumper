...
pub async fn jpg_resize(ratios: &[(u32, &str)], src: &SourceFile, img: &DynamicImage,
resizer: &mut Resizer, s3: &Objects) -> Result<ResizeStats> {
    // Preprocessing for image. Check if all required ratios are present and
    // match the input file.
    let resizables = src.get_resizables(ratios, s3, "jpeg").await?;
    ...
    for (r, s, p) in resizables {
        ...
        handles.spawn(async move {
            ...
            jpeg_writer(&p, dst_image, &s3).await?;
...
pub async fn jpg_crop_resize(ratios: &[(u32, &str)], src: &SourceFile, img: &DynamicImage,
resizer: &mut Resizer, s3: &Objects) -> Result<ResizeStats> {
    let resizables = src.get_resizables(ratios, s3, "jpeg").await?;
    let mut stats = ResizeStats::new();
...
async fn jpeg_writer(file_path: &PathBuf, image: Image<'_>, s3: &Objects) -> Result<()> {
